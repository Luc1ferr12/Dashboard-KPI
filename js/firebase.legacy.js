...
const db = firebase.firestore(); // Gunakan firebase.firestore()
// const storage = firebase.storage(); // Tidak lagi menggunakan Storage
...
async function uploadExcelFile(file, pageId) {
    try {
        ...
        const docRef = await db.collection(pageId + 'Data').add({
            name: file.name,
            content: base64Content, // Simpan konten sebagai string Base64
            timestamp: firebase.firestore.FieldValue.serverTimestamp() // Gunakan serverTimestamp untuk Firestore
        });
...
async function fetchExcelData(pageId) {
    try {
        const dataCollection = db.collection(pageId + 'Data');
        // Query untuk mendapatkan dokumen terbaru berdasarkan timestamp
        const querySnapshot = await dataCollection.orderBy('timestamp', 'desc').limit(1).get();
...
async function downloadExcelFile(data) { // data diharapkan berupa objek { name: string, content: string (base64) }
    try {
